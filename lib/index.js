...
function _getRoutes() {
	...
	//api or page
	Object.keys(controllers).forEach(function(controllerDir) {
		if (controllerDir === 'api'){
			Object.keys(controllers[controllerDir]).forEach(function(controller) {
				var currController = controllers[controllerDir][controller];
				var currName = controller.slice(0, -10);
				// currController._config = currController._config || {};
				// currController._config['prefix'] = controllerDir;
				//Rename the key to the controller to be '[Resource]' instead of '[Resource]Controller'
				var t = controllerDir + "/" + currName.lowerFirst(); //minus 10 to strip out 'Controller' from the file name
				controllers[t] = currController;
				delete controllers[controllerDir][controller];
				//Add the resource
				resourceNames = resourceNames.concat(t);
			});
		}
		delete controllers[controllerDir];
	});
	...
	resourceNames.forEach(function(resourceName) {
		...
		var defaultPrefix = (prefix ? prefix : '') + (plural ? pluralize(resourceName).toLowerCase() : resourceName.toLowerCase()) //TODO: pluralize
		...
		if (actions) {
			...
			Object.keys(controller).forEach(function(actionName) {
				if (_.contains(['globalId', 'identity', 
				'getDataListByPage', 'sails', '_config'], actionName)) return;
	...
	Object.keys(customRoutes).forEach(function(_path) {
		if (_path.indexOf('post /api/') !== 0 &&
			_path.indexOf('get /api/') !== 0)
			return;
		//TODO handle casing in _path (for example POST and post)
		var target = customRoutes[_path];
		_path = _path.replace('/api/', 'api/');
		target.path = _path;
		if (typeof target === 'string') {
			...
			if (target.controller){
				var currName = target.controller.slice(0, -10);
				target.resource = 'api/'+currName.replace('api/','').lowerFirst();
			}
		}
		var delPath = _path.replace('/:type','');
		delete resourceRoutes[delPath];
		resourceRoutes[_path] = target;
	});
...
function _mergeDocs() {
	...
	Object.keys(_routes).forEach(function(_path) {
		if (!_routes[_path].path)
			_routes[_path].path = _path;
	});
	...
	Object.keys(resources).forEach(function(resourceName) {
		...
		var resourceDocs = [];
		var routes = [];
		var classDoc = _.find(_controllerDocs[resourceName], function(doc) {
		...
		Object.keys(resource).forEach(function(key) {
			...
			if (_.contains(['getDataListByPage', 'sails', '_config'], route.action))
				return;
		...
		if (routes.length > 0) {
			...
			var tmpResourceName = resourceName;

			if (resourceName.indexOf('api/') === 0 ){
				tmpResourceName = resourceName.replace('api/', '').capitalize();
			} else if (resourceName.indexOf('page/') === 0){
				tmpResourceName = 'Page-'+resourceName.replace('page/', '').capitalize();
			}

			if (_modelDocs[tmpResourceName]) 
				resourceDocs = resourceDocs.concat(_modelDocs[tmpResourceName]);

			if (classDoc)
				resourceDocs = resourceDocs.concat(classDoc);

			result = result.concat([{
				name: tmpResourceName,
				dox: resourceDocs,
				routes: routes
			}]);
		}
...
function _resourceFromFile(fname) {
	...
	name = t > -1 ? name.slice(0, t) : name;
	t = name.indexOf('\\');
	if (t > 0) {
		var names = name.split('\\');
		name = names[0] + '/' + names[1].lowerFirst();
	}
	return name;
}